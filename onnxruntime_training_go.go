...
// A scalar is like a tensor but the underlying go slice is of length 1 and it has no dimension.
...
// ExportModel is used to export the model to disk. It requires the path for
...
func (s *TrainingSession) ExportModel(path string, outputNames []string) error {
	if s.evalModelPath == nil {
		return fmt.Errorf("final model can only be exported if the eval model is provided at session creation time (see NewTrainingSession)")
	}
	if path == "" {
